...
lox = randi([1,n],n,2);
proc_delay_1=2.29/10000
proc_delay_2=2.29
proc_delay_3=2.29/10000

% sc1: fully cent.
% 1 fast MP+processing+1 fast MP+ processing
delay_1=((0.0033)*2+proc_delay_1*k*n)*10^-1;

% sc2: fully decent.
% 1 slow MP+processing+ 1 slow MP+ processing
delay_2=((0.017).*k*2*10+proc_delay_2*k)*10^-1;

% sc3: hybrid -fixed assign.
% 1 slow MP+processing+ 1 slow MP+ processing
cloudlet_size=25;
costs=comm_cost(250, cloudlet_size, lox);
delay_31=(sum(costs)/250/10.*k/1000+proc_delay_3*k*10)*10^-1;
...
% my_fig_trim_export('inf_time_comp_ccrnn2')
...
imagesc(all)
axis off;
my_fig_trim_export('adaptive_assignment_ccrnn')
...
imagesc(all)
axis off;
my_fig_trim_export('uniform_assignment_ccrnn')
