...
delay_31=sum(1*(1).*costs*2)/250/10.*k;

% sc3: hybrid -fixed assign.
% 1 slow MP+processing+ 1 slow MP+ processing
cloudlet_size=50;
costs=comm_cost(250, cloudlet_size, lox);
delay_32=sum(1*(1).*costs*2)/250/10.*k;
...
delay_41=sum(1*(1).*costs*2)/250/10.*k;

% sc4: hybrid-adaptive assign.
cloudlet_size=20;
costs=comm_cost_adaptive(250, cloudlet_size, lox);
delay_42=sum(1*(1).*costs*2)/250/10.*k;
...
grid
set(gca,'xtick',1:5)
legend('SC1', 'SC2', 'SC3(10)','SC3(20)','SC4(10)','SC4(20)','location', 'best',  'FontSize', 9)
title('Inference Time Comparison')
ylabel('Delay (ms)')
xlabel('Communication hops')
my_fig_trim_export('inf_time_comp.pdf')
...
% imagesc(heatmap);
% axis off;
my_fig_trim_export('unfiform_assignment.pdf')
...
imagesc(all)
axis off;
my_fig_trim_export('adaptive_assignment.pdf')
...
for i=1:n
    cost(i)= cost_clusters(lox(i,1), lox(i,2));
end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
